...
def deploy():
    ...
    deployment_labels = data.get('deployment_labels')
    file_url = data.get('file_url')

    if not all([project_name, deployment_description, deployment_labels, deployment_name, deployment_version, file_url]):
        return jsonify({'error': 'Missing required parameters'}), 400

    try:
        # Debug: Print the API token to verify it's loaded
        print(f"UBI_OPS_API_TOKEN: {UBI_OPS_API_TOKEN}")
